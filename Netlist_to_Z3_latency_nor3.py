...
def generate_Z3_file(name, D, types, InputString, OutputString, circuit_output_gates, numrows=1000, numcols=1000):
    ...
    opt.write('(minimize(+ Latency 0))\n')
    opt.write('(check-sat)\n')
    opt.write('(get-model)\n')
    opt.close()
    return optfilename 

def netlist_to_z3(filename):
    ...
    for ii in range(len(BenchmarkStrings)):
        ...
        return generate_Z3_file(BenchmarkStrings[ii], D, types, InputString, OutputString, circuit_output_gates, r, r)
netlist_to_z3("cm150a_nor3.txt")