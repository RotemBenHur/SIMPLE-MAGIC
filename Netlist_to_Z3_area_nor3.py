...
def generate_Z3_file(name, D, types, InputString, latency, numrows=1000, numcols=1000):
    ...
    opt.write('(declare-const Area Int)\n')
    opt.write('(declare-const minRow Int)\n')
    opt.write('(declare-const maxRow Int)\n')
    opt.write('(declare-const minCol Int)\n')
    opt.write('(declare-const maxCol Int)\n')
    opt.write('\n')
    ...
    for i in xrange(1, gate_num + 1):
        for j in xrange(i+1, gate_num + 1):
            if types[i-1] == types[j-1] and types[i-1] == 3: # If both gates are NOR3
                opt.write('(assert\n')
                opt.write('  (or (not(= g%d_T g%d_T))\n' %  (i,j))
    ...
    opt.write(';# Area constraints\n')
    opt.write('(assert (and \n')
    for i in xrange(1, gate_num + 1):
        if types[i-1] == 1:
            opt.write('(<= minRow g%d_Rin1) (<= minRow g%d_Rout)\n' % (i,i))
        elif types[i-1] == 2:
            opt.write('(<= minRow g%d_Rin1) (<= minRow g%d_Rin2) (<= minRow g%d_Rout)\n' % (i, i, i))
        elif types[i-1] == 3:
            opt.write('(<= minRow g%d_Rin1) (<= minRow g%d_Rin2) (<= minRow g%d_Rin3) (<= minRow g%d_Rout)\n' % (i, i, i, i))
    for i in xrange(len(InputString)):
        opt.write('(<= minRow %s_Row)\n'% InputString[i])
    opt.write(')) \n')

    opt.write('(assert (and \n')
    for i in xrange(1, gate_num + 1):
        if types[i-1] == 1:
            opt.write('(>= maxRow g%d_Rin1) (>= maxRow g%d_Rout)\n' % (i, i))
        elif types[i-1] == 2:
            opt.write('(>= maxRow g%d_Rin1) (>= maxRow g%d_Rin2) (>= maxRow g%d_Rout)\n' % (i, i, i))
        elif types[i-1] == 3:
            opt.write('(>= maxRow g%d_Rin1) (>= maxRow g%d_Rin2) (>= maxRow g%d_Rin3) (>= maxRow g%d_Rout)\n' % (i, i, i, i))
    for i in xrange(len(InputString)):
        opt.write('(>= maxRow %s_Row)\n'% InputString[i])
    opt.write(')) \n')

    opt.write('(assert (and \n')
    for i in xrange(1, gate_num + 1):
        if types[i-1] == 1:
            opt.write('(<= minCol g%d_Cin1) (<= minCol g%d_Cout)\n' % (i, i))
        elif types[i-1] == 2:
            opt.write('(<= minCol g%d_Cin1) (<= minCol g%d_Cin2) (<= minCol g%d_Cout)\n' % (i, i, i))
        elif types[i-1] == 3:
            opt.write('(<= minCol g%d_Cin1) (<= minCol g%d_Cin2) (<= minCol g%d_Cin3) (<= minCol g%d_Cout)\n' % (i, i, i, i))
    for i in xrange(len(InputString)):
        opt.write('(<= minCol %s_Col)\n'% InputString[i])
    opt.write(')) \n')

    opt.write('(assert (and \n')
    for i in xrange(1, gate_num + 1):
        if types[i-1] == 1:
            opt.write('(>= maxCol g%d_Cin1) (>= maxCol g%d_Cout)\n' % (i, i))
        elif types[i-1] == 2:
            opt.write('(>= maxCol g%d_Cin1) (>= maxCol g%d_Cin2) (>= maxCol g%d_Cout)\n' % (i, i, i))
        elif types[i-1] == 3:
            opt.write('(>= maxCol g%d_Cin1) (>= maxCol g%d_Cin2) (>= maxCol g%d_Cin3) (>= maxCol g%d_Cout)\n' % (i, i, i, i))
    for i in xrange(len(InputString)):
        opt.write('(>= maxCol %s_Col)\n'% InputString[i])
    opt.write(')) \n')
    opt.write('\n')

    opt.write('(assert (= Area (* (+ (- maxRow minRow) 1) (+ (- maxCol minCol) 1))))\n')
    opt.write('(assert (>= Area %d))\n' % (gate_num + len(InputString)))    

    opt.write(';## Objective function\n')
    opt.write('(assert (= Latency %d))\n' % latency)
    opt.write('(minimize(+ Area 0))\n')
    opt.write('(check-sat)\n')
    opt.write('(get-model)\n')
    opt.close()
    return optfilename 
